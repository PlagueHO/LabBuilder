Configuration ROOTCA
{
	...
	Node $AllNodes.NodeName {
		...
		Script ADCSAdvConfig
		{
			...
			TestScript = { 
				Add-Content -Path "$ENV:Temp\TestScript.log" -Value ((Get-ChildItem 'HKLM:\System\CurrentControlSet\Services\CertSvc\Configuration').GetValue('DSConfigDN'))
				If (($Node.DSConfigDN) -and ((Get-ChildItem 'HKLM:\System\CurrentControlSet\Services\CertSvc\Configuration').GetValue('DSConfigDN') -ne $Node.DSConfigDN)) {
					Add-Content -Path "$ENV:Temp\TestScript.log" -Value "DSConfigDN NE"
					Return $False
				}
				Add-Content -Path "$ENV:Temp\TestScript.log" -Value ((Get-ChildItem 'HKLM:\System\CurrentControlSet\Services\CertSvc\Configuration').GetValue('CRLPublicationURLs'))
				If (($Node.CRLPublicationURLs) -and ((Get-ChildItem 'HKLM:\System\CurrentControlSet\Services\CertSvc\Configuration').GetValue('CRLPublicationURLs') -ne $Node.CRLPublicationURLs)) {
					Add-Content -Path "$ENV:Temp\TestScript.log" -Value "CRLPublicationURLs NE"
					Return $False
				}
				Add-Content -Path "$ENV:Temp\TestScript.log" -Value ((Get-ChildItem 'HKLM:\System\CurrentControlSet\Services\CertSvc\Configuration').GetValue('CACertPublicationURLs'))
				If (($Node.CACertPublicationURLs) -and ((Get-ChildItem 'HKLM:\System\CurrentControlSet\Services\CertSvc\Configuration').GetValue('CACertPublicationURLs') -ne $Node.CACertPublicationURLs)) {
					Add-Content -Path "$ENV:Temp\TestScript.log" -Value "CACertPublicationURLs NE"
					Return $False
				}
				Return $True
			}
